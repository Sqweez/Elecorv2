    ...
    <v-card>
        ...
        <v-card-text>
            <div class="clients-details">
                <h4>Всего клиентов: {{ clientsCount }}</h4>
                <v-btn
                    color="primary"
                    class="button-add"
            ...
            >
                <template v-slot:item.personalAccount="{ item }">
                    ...
                    <ul>
                        <li v-for="(i, index) of item.connections" :key="index">
                            {{ i.personal_account }}
                        </li>
                    </ul>
                </template>
        ...
        <AddClientModal
            :state="showAddClientModal"
            v-on:onSave="saveUser"
            v-on:onClose="closeModal"
        />
        <ImportClientsModal
            :state="showImportModal"
            v-on:onClose="showImportModal = false"
            v-on:onSave="onClientImported"
        />
        <ParseBalanceModal
            :state="showParseBalanceModal"
            v-on:onClose="showParseBalanceModal = false"
            v-on:onSave="onBalanceImported"
        />
        <ExportClientsModal
            :state="exportClientModal"
            v-on:cancel="exportClientModal = false"
            v-on:submit="onExportSubmit"
        />
    </v-card>
</template>
...
export default {
    ...
    computed: {
        ...
        clients() {
            if (this.client_type === -1) {
                ...
                return this.$store.getters[GETTERS.CLIENTS].filter(
                    client => client.client_type === this.client_type
                )
            }
        },
        subjects() {
    ...
    methods: {
        ...
        async onExportSubmit(query) {
            ...
            link.href = `/api/export/clients?${query}`
